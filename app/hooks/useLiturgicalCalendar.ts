...
export function useLiturgicalCalendar(date: Date = new Date()): LiturgicalInfo {
  return useMemo(() => {
    ...
    const sundayLocal = new Date(sundayInProgress.getFullYear(), sundayInProgress.getMonth(), sundayInProgress.getDate());
    ...
    if (!foundNamedSunday) {
      if (weekAfterPascha !== null) {
        weekName = i18n.t('weekNames.afterPascha', { count: weekAfterPascha });
      } else if (weekAfterPentecost !== null) {
        weekName = i18n.t('weekNames.afterPentecost', { count: weekAfterPentecost });
      }
    }
    ...
    if (dates.nativityFast && typeof dates.nativityFast === 'object') {
      ...
      if ('followingYear' in dates.nativityFast && dates.nativityFast.followingYear && isRange(dates.nativityFast.followingYear)) {
    ...
    const todayLocalDate = new Date(date.getFullYear(), date.getMonth(), date.getDate());
    ...
    const specialDays = [
      ...
      { key: 'nativityTheotokos', label: i18n.t('liturgical.nativityTheotokos') },
      { key: 'exaltationCross', label: i18n.t('liturgical.exaltationCross') },
      { key: 'presentationTheotokos', label: i18n.t('liturgical.presentationTheotokos') },
      ...
      { key: 'forgivenessSunday', label: i18n.t('liturgical.forgivenessSunday') },
      ...
      { key: 'presentationChrist', label: i18n.t('liturgical.presentationChrist') },
      { key: 'beheadingBaptist', label: i18n.t('liturgical.beheadingBaptist') },
      { key: 'protectionTheotokos', label: i18n.t('liturgical.protectionTheotokos') },
      { key: 'sundayOfOrthodoxy', label: i18n.t('liturgical.sundayOfOrthodoxy') },
    ];
    ...
    for (const day of specialDays) {
      ...
      if (dateValue && isDate(dateValue)) {
        ...
        const feastLocalDate = new Date(feastDate.getFullYear(), feastDate.getMonth(), feastDate.getDate());
    ...
    if (!specialDay && dates.nativity && isDate(dates.nativity)) {
      ...
      const nativityLocalDate = new Date(nativityDate.getFullYear(), nativityDate.getMonth(), nativityDate.getDate());
